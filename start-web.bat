...
if errorlevel 1 (
  ...
  echo Please close that process manually, then run start-web.bat again.
  pause
  exit /b 1
)
...
:port_ready
echo [INFO] Starting local web GUI...
call npm run gui
set EXIT_CODE=%ERRORLEVEL%
if not %EXIT_CODE%==0 (
  echo [ERROR] GUI server stopped with exit code %EXIT_CODE%.
  pause
)
exit /b %EXIT_CODE%
