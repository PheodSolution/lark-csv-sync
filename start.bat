...
if not exist ".env" (
  ...
  echo Create .env and set LARK_APP_ID / LARK_APP_SECRET.
)
...
if errorlevel 1 (
  ...
  echo Please close that process manually, then run start.bat again.
  pause
  exit /b 1
)
...
if not %EXIT_CODE%==0 (
  echo [ERROR] Desktop app failed with exit code %EXIT_CODE%.
)
pause
exit /b %EXIT_CODE%
